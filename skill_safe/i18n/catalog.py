...
TAXONOMY_MESSAGES: dict[str, dict[str, dict[str, str]]] = {
    "zh": {
        ...
        "CH-001.title": "不可信字符串流向 shell 风险",
        "CH-001.impact": "不可信字符串可能经工具输出或提示链路进入 shell 执行面。",
        "CH-001.remediation": "阻断文本到命令执行的直连路径，并对 shell 参数做结构化校验。",
        "CH-002.title": "不可信字符串流向上下文风险",
        "CH-002.impact": "不可信内容可能被写入系统提示、记忆或长期上下文，形成间接注入。",
        "CH-002.remediation": "不要把不可信输出直接提升为系统上下文或持久记忆。",
        "CH-003.title": "不可信字符串流向敏感动作参数风险",
        "CH-003.impact": "不可信内容可能控制删除路径、网络目标或其他高敏感动作参数。",
        "CH-003.remediation": "对敏感动作参数实行 allowlist 和结构化校验。",
        "CH-004.title": "敏感信息流向外部出口风险",
        "CH-004.impact": "本地读取到的 secrets 可能通过上传、同步或外部请求离开宿主。",
        "CH-004.remediation": "将 secrets 读取与外部出口隔离，并默认阻断该类组合链路。",
        ...
        "diff.old_target": "旧版本目标",
        "diff.new_target": "新版本目标",
        "diff.old_decision": "旧版本结论",
        "diff.new_decision": "新版本结论",
        "diff.decision_changed": "结论是否变化",
        "diff.added_taxonomy_ids": "新增风险分类",
        "diff.removed_taxonomy_ids": "移除风险分类",
        "diff.permission_drift": "权限漂移",
        "diff.flow_drift": "攻击链漂移",
        "diff.added_flow_ids": "新增攻击链 ID",
        "diff.removed_flow_ids": "移除攻击链 ID",
        "diff.added_flow_taxonomy_ids": "新增攻击链分类",
        "diff.removed_flow_taxonomy_ids": "移除攻击链分类",
        "diff.trust_profile_drift": "信任画像漂移",
        "explain.kind": "解释类型",
        "explain.headline": "摘要结论",
        "explain.narrative": "解释",
        "explain.key_findings": "重点发现",
        "explain.key_flows": "重点攻击链",
        "explain.key_changes": "重点变化",
        "explain.recommended_actions": "建议动作",
        "explain.scan.headline.allow": "当前报告未显示阻断级风险，但仍建议在启用前做最小权限确认。",
        "explain.scan.headline.review": "当前报告存在需要人工复核的风险或语义偏差。",
        "explain.scan.headline.block": "当前报告命中了阻断级风险，不建议直接启用该 skill。",
        "explain.scan.headline.sandbox_only": "当前报告建议仅在隔离环境中运行该 skill。",
        "explain.scan.narrative.findings": "本次扫描共发现 {count} 个问题信号。",
        "explain.scan.narrative.flows": "本次扫描识别出 {count} 条潜在攻击链。",
        "explain.scan.narrative.decision": "当前准入结论为 {decision}。原因：{reason}",
        "explain.diff.headline.changed": "新旧版本之间存在实质性安全变化，需要重新审计。",
        "explain.diff.headline.stable": "新旧版本的总体准入结论保持稳定。",
        "explain.diff.narrative.decision": "准入结论从 {old} 变化为 {new}。",
        "explain.diff.narrative.taxonomy": "新增风险分类：{added}；移除风险分类：{removed}。",
        "explain.diff.narrative.flows": "新增攻击链：{added}；移除攻击链：{removed}。",
        "explain.action.blocked": "保持阻断状态，不要直接在宿主环境启用该 skill。",
        "explain.action.isolate": "若必须验证，请仅在隔离沙箱或一次性环境中进行。",
        "explain.action.rotate": "检查并轮换可能暴露的凭据、token 和敏感配置。",
        "explain.action.review": "结合权限、来源和行为证据做一次人工复核。",
        "explain.action.reaudit": "把新版本视为新的准入对象，重新执行完整审计流程。",
        "explain.action.recheck_permissions": "重点复核新增 taxonomy 对应的权限与执行面。",
        "explain.action.recheck_flows": "重点复核新增攻击链是否改变了上游输入到危险 sink 的连通关系。",
        "explain.action.compare_permissions": "比较升级前后的权限、启动项和外联面变化。",
    },
    "en": {
        ...
        "CH-001.title": "Untrusted string to shell risk",
        "CH-001.impact": "Untrusted text may cross a tool or prompt boundary and reach shell execution surfaces.",
        "CH-001.remediation": "Block direct text-to-command paths and validate shell parameters structurally.",
        "CH-002.title": "Untrusted string to context risk",
        "CH-002.impact": "Untrusted content may be promoted into system prompts, memory, or long-lived context.",
        "CH-002.remediation": "Do not upgrade untrusted output into persistent or privileged context.",
        "CH-003.title": "Untrusted string to sensitive parameter risk",
        "CH-003.impact": "Untrusted content may control delete targets, network destinations, or other sensitive action parameters.",
        "CH-003.remediation": "Enforce allowlists and structured validation on sensitive action parameters.",
        "CH-004.title": "Sensitive material to external sink risk",
        "CH-004.impact": "Secrets read locally may leave the host through upload, sync, or outbound request paths.",
        "CH-004.remediation": "Separate secrets access from egress paths and block this combination by default.",
        ...
        "diff.old_target": "Old target",
        "diff.new_target": "New target",
        "diff.old_decision": "Old decision",
        "diff.new_decision": "New decision",
        "diff.decision_changed": "Decision changed",
        "diff.added_taxonomy_ids": "Added taxonomy IDs",
        "diff.removed_taxonomy_ids": "Removed taxonomy IDs",
        "diff.permission_drift": "Permission drift",
        "diff.flow_drift": "Flow drift",
        "diff.added_flow_ids": "Added flow IDs",
        "diff.removed_flow_ids": "Removed flow IDs",
        "diff.added_flow_taxonomy_ids": "Added flow taxonomy IDs",
        "diff.removed_flow_taxonomy_ids": "Removed flow taxonomy IDs",
        "diff.trust_profile_drift": "Trust profile drift",
        "explain.kind": "Explanation type",
        "explain.headline": "Headline",
        "explain.narrative": "Narrative",
        "explain.key_findings": "Key findings",
        "explain.key_flows": "Key flows",
        "explain.key_changes": "Key changes",
        "explain.recommended_actions": "Recommended actions",
        "explain.scan.headline.allow": "The report does not show a blocking issue, but a least-privilege review is still recommended before enablement.",
        "explain.scan.headline.review": "The report contains risks or semantic mismatches that require human review.",
        "explain.scan.headline.block": "The report hits blocking-risk categories and the skill should not be enabled directly.",
        "explain.scan.headline.sandbox_only": "The report recommends running the skill only inside a restricted sandbox.",
        "explain.scan.narrative.findings": "This scan identified {count} issue signals.",
        "explain.scan.narrative.flows": "This scan identified {count} potential attack chains.",
        "explain.scan.narrative.decision": "The current admission decision is {decision}. Reason: {reason}",
        "explain.diff.headline.changed": "The two versions differ materially from a security standpoint and require re-audit.",
        "explain.diff.headline.stable": "The overall admission decision remains stable across the two versions.",
        "explain.diff.narrative.decision": "The admission decision changed from {old} to {new}.",
        "explain.diff.narrative.taxonomy": "Added taxonomy IDs: {added}; removed taxonomy IDs: {removed}.",
        "explain.diff.narrative.flows": "Added flow IDs: {added}; removed flow IDs: {removed}.",
        "explain.action.blocked": "Keep the skill blocked and do not enable it directly on the host.",
        "explain.action.isolate": "If validation is necessary, run it only inside a sandbox or disposable environment.",
        "explain.action.rotate": "Check and rotate any credentials, tokens, or sensitive configuration that may have been exposed.",
        "explain.action.review": "Perform a human review across permissions, provenance, and behavior evidence.",
        "explain.action.reaudit": "Treat the new version as a fresh admission candidate and run a full audit again.",
        "explain.action.recheck_permissions": "Review the newly added taxonomy categories against the exposed permissions and execution surface.",
        "explain.action.recheck_flows": "Review whether newly added attack chains changed how upstream input can reach dangerous sinks.",
        "explain.action.compare_permissions": "Compare the pre/post-upgrade permissions, startup hooks, and egress surface.",
    },
}
